...
3. Meta returns to the canonical existing callback at `/auth/callback`. That route first checks whether the state belongs to a v2 attempt and dispatches to the v2 flow before applying legacy v1 validation. `/auth/instagram/callback` and `/api/onboarding/auth/callback` remain compatibility/drain aliases. The v2 callback validates state and browser binding, encrypts and checkpoints the raw one-time code, invalidates state, and immediately issues a clean `303` to `/connecting?attemptId=...`. It performs no token exchange.
...
- Production, and staging when Supabase-backed, require an exact canonical Supabase project URL plus matching expected Supabase ref and actual/expected Vercel project ID before a client is constructed. The expected ref is read from `SUPABASE_EXPECTED_PROJECT_REF` with legacy `SUPABASE_PRODUCTION_PROJECT_REF` fallback, and conflicting configured refs fail closed. Server database access accepts only `sb_secret_*` or JWT `service_role` keys from `SUPABASE_SECRET_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, or legacy `SUPABASE_KEY`; conflicting configured values fail closed.
...
The legacy state/cookie format remains accepted during the transition. Its persistence adapter must decide under the same account lock whether v2 already owns the account; a JavaScript read followed by an old RPC is not sufficient. A successful legacy callback is normalized to `/legacy-complete` with a five-minute, encrypted, browser-bound HttpOnly receipt, so neither the obsolete `/Dashboard` path nor an unauthorized `/complete` page is exposed. Application rollback keeps the v2 callback/API drain path and additive schema available. Forward migration and rollback SQL live in separate directories so normal migration tooling cannot apply rollback automatically.
